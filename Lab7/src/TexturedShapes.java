...
public class TexturedShapes {
    ...
    public static void pyramide(GL2 gl){ pyramide(gl,0.7f,5,0.7f);}
    ...
    public static void pyramidewall(GL2 gl2, float a1,float a2, float b1, float b2, float c1 ,float c2,float h) {

        gl2.glBegin(GL_TRIANGLE_FAN);
        gl2.glTexCoord2d(0,0);
        gl2.glVertex3f(a1,  0,a2);
        gl2.glTexCoord2d(1,1);
        gl2.glVertex3f(b1, h,b2);
        gl2.glTexCoord2d(0,1);
        gl2.glVertex3f(c1,  0,c2);
        gl2.glTexCoord2d(0,1);
        gl2.glEnd();
    }

    public static void triangle(GL2 gl2, float a1,float a2, float b1, float b2, float c1 ,float c2) {

        gl2.glBegin(GL_TRIANGLE_FAN);
        gl2.glTexCoord2d(0,0);
        gl2.glVertex3f(a1,  0f,a2);
        gl2.glTexCoord2d(1,1);
        gl2.glVertex3f(b1, 0f,b2);
        gl2.glTexCoord2d(0,1);
        gl2.glVertex3f(c1,  0f,c2);
        gl2.glTexCoord2d(0,1);
        gl2.glEnd();
    }

    public static void pyramide(GL2 gl2, float r, int N,float h) {
        float[] xPoints = new float[N];
        float[] yPoints = new float[N];
        for(int i = 1 ; i<=N;i++) {
            xPoints[i - 1] = (float) (r * Math.sin((2 * Math.PI / N) * i));
            yPoints[i - 1] = (float) (r * Math.cos((2 * Math.PI / N) * i));
        }
        gl2.glPushMatrix(); // Save a copy of the current matrix.
        for(int i = 0 ; i<N-1;i++)
        {
            gl2.glPushMatrix();
            gl2.glRotatef(360/N,0,1,0);
            triangle(gl2,xPoints[i], yPoints[i],0,0, xPoints[i+1], yPoints[i+1]);
            pyramidewall(gl2,xPoints[i], yPoints[i],0,0, xPoints[i+1], yPoints[i+1],h);
            gl2.glPopMatrix();
        }
        gl2.glPushMatrix();
        gl2.glRotatef(360/N,0,1,0);
        triangle(gl2,xPoints[N-1], yPoints[N-1],0,0, xPoints[0], yPoints[0]);
        pyramidewall(gl2,xPoints[N-1], yPoints[N-1],0,0, xPoints[0], yPoints[0],h);
        gl2.glPopMatrix();
    }
}
